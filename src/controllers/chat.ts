  ...
  try {
    const messages = await Chat.find({
      $or: [
        ...
        { conversationId: `${req.query.mate}-${req.user?._id}` },
      ],
...
export const saveMessage = expressAsyncHandler(async (req: any, res: any) => {
    ...
    try {
      ...
      chats.forEach((item) => {
        ...
        if (members.includes(req.user?._id.valueOf())) {
          const findSimilar = discussions.find(
            (item: any) => item.members.toString() == members.toString()
          );
          const findSimilar_c = discussions.find(
            (item: any) => item.members.toString() == members_c.toString()
          );
